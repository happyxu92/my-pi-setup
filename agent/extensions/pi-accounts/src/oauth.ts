...
export type ProviderOwnedOAuth = Pick<
  OAuthAuth,
  "login" | "refresh" | "toAuth"
>;
  ...
  return [
    ...
    },
    {
      ...
      resolveOAuth: (ctx) => {
        ...
        if (!oauth)
          throw new Error("Pi's active Radius OAuth provider is unavailable.");
        return oauth;
      },
...
export async function loginWithOAuthUI(
  ...
  if (ctx.mode !== "tui") {
    return oauth.login(
      createOAuthInteraction(ctx, adapter.displayName, signal),
    );
  }
  const result = await ctx.ui.custom<NativeOAuthLoginResult>(
...
type NativeOAuthLoginResult =
  { ok: true; credential: OAuthCredential } | { ok: false; error: unknown };
...
class NativeOAuthLoginFlow {
  ...
  constructor(
    tui: ConstructorParameters<typeof LoginDialogComponent>[0],
    adapter: AccountProviderAdapter,
    oauth: ProviderOwnedOAuth,
    ownerSignal: AbortSignal,
    done: (result: NativeOAuthLoginResult) => void,
  ) {
    this.tui = tui;
    this.oauth = oauth;
    this.done = done;
  ...
  private async prompt(prompt: AuthPrompt): Promise<string> {
    ...
    const signal = prompt.signal
      ? AbortSignal.any([this.signal, prompt.signal])
      : this.signal;
...
function setComponentFocus(
  component: NativeOAuthComponent,
  focused: boolean,
): void {
