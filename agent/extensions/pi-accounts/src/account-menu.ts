...
import { showProjectDefaultsMenu } from "./project-default-menu.ts";
import {
  type AccountStore,
  defineOwnMap,
  getOwnCredential,
...
import type { ProviderAccountSelections } from "./session-selection.ts";
...
export async function showAccountsMenu(
  ...
  const menu = defineMenu<State, Screen, Action, ExtensionCommandContext>({
    ...
    screens: {
        ...
        return {
          ...
          items: [
            ...
            ),
            {
              ...
              description:
                "For new sessions only; keeps this session unchanged",
              action: "default-route",
            },
            {
              id: "project-default",
              label: "Set project default account",
              description:
                "For new sessions in this trusted working directory only",
              action: "project-default-route",
            },
          ],
          hint: "close",
        };
      ...
      "switch-accounts": ({ state }) => {
        const provider = selectedProviderId
          ? state.states.get(selectedProviderId)
          : undefined;
        ...
        return {
          ...
          items: options.map((option) => {
            ...
            return {
              ...
              disabled:
                !provider?.selectionInvalid &&
                accountName === (provider?.active ?? "default"),
            };
          }),
          hint: "back",
        };
      ...
      "default-providers": ({ state }) => ({
        ...
        lines: [
          "User-wide settings. Existing sessions keep their own accounts.",
        ],
        items: sortedProviderStates(state.states).map((provider) => ({
          id: provider.id,
          label: provider.adapter.displayName,
      ...
      remove: ({ state }) => ({
        ...
        items: removeAccountOptions(state.states, state.currentProviderId).map(
          (option) => ({
    ...
    actions: {
      "project-default-route": async ({ signal }) => {
        await showProjectDefaultsMenu(ctx, store, adapters, {
          signal,
          isCurrent: () => owner.isCurrent() && !signal.aborted,
        });
        return { kind: "close" };
      },
      "default-route": async () => ({
        kind: "to",
        screen: "default-providers",
      }),
      ...
      "default-account": async ({ itemId, signal }) => {
        ...
        try {
          const saved = await store.updateProvider(providerId, (state) => {
            if (!isCurrent())
              throw new DOMException("Menu closed", "AbortError");
            const name = Object.keys(state.accounts).find(
              (name) => accountItemId(name) === itemId,
            );
            if (itemId !== "pi-login" && !name)
              throw new Error("Account no longer exists");
            return {
              ...state,
              active: itemId === "pi-login" ? undefined : name,
            };
          });
          if (!isCurrent()) return { kind: "close" };
          ctx.ui.notify(
      ...
      "login-provider": async ({ itemId, signal }) => {
        ...
        const name = await ctx.ui.input(
          `Name this ${adapter.displayName} account:`,
          "work",
          {
            signal,
          },
      ...
      "remove-account": async ({ itemId, signal }) => {
        ...
        if (!owner.isCurrent()) return { kind: "close" };
        const option = removeAccountOptions(
          states,
          toProviderId(ctx.model?.provider),
        ).find(
          (candidate) =>
            removeAccountItemId(candidate.adapter.id, candidate.accountName) ===
            itemId,
        );
  ...
  await runMenu(ctx, menu, {
    getState: async () => {
      const states = await readProviderMenuStates(store, adapters, session);
      const missingSelection = [...states.values()].some(
        (state) => state.selectionInvalid,
      );
      return {
        states,
        currentProviderId: toProviderId(ctx.model?.provider),
        hasAnyStoredAccount: [...states.values()].some(
...
function formatAccountsMenuTitle(
  ...
  return [
    "Accounts",
    ...(selectionError
      ? ["", `Selection error: ${sanitizeTerminalText(selectionError)}`]
      : []),
    "",
    "Current model:",
        ...
        ? [
            {
              id: "remove",
              label: REMOVE_ACTION,
              action: "remove-route" as const,
            },
          ]
        : []),
      ...(providerStatesWithAccounts(states, currentState.id, selectionInvalid)
        .length > 0
        ? [
            {
              id: "switch-other",
...
function sortedProviderStates(
  states:
    Map<AccountProviderId, ProviderMenuState> | readonly ProviderMenuState[],
): ProviderMenuState[] {
...
function providerStatesWithAccounts(
  ...
  return sortedProviderStates(states).filter(
    (state) =>
      state.id !== excludeProviderId &&
      (includeEmpty || accountNames(state).length > 0),
  );
}
