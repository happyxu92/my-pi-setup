...
import { defineOwn, defineOwnMap, parseAccountName } from "./account-store.ts";
...
export function restoreAccountSelections(
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    ...
    if (
      entry?.type !== "custom" ||
      entry.customType !== ACCOUNT_SELECTION_ENTRY_TYPE
    )
      continue;
    ...
    try {
      return {
        status: "loaded",
        selections: parseSelectionEntryData(entry.data).providers,
      };
    } catch {
      return {
        status: "invalid",
...
export function createAccountSelectionEntryData(
  ...
  if (!sessionId)
    throw new Error(
      "Cannot persist account selection without a Pi session ID.",
    );
  return {
...
function parseSelectionEntryData(
  ...
  if (!isRecord(value.providers))
    throw new Error("Invalid account selections.");
  return {
    version: ACCOUNT_SELECTION_VERSION,
...
function normalizeSelections(
  ...
  if (entries.length > MAX_PROVIDER_SELECTIONS)
    throw new Error("Too many account selections.");
  ...
  for (const [providerId, accountName] of entries) {
    if (!PROVIDER_ID_RE.test(providerId))
      throw new Error("Invalid account selection provider.");
    if (accountName === null) {
