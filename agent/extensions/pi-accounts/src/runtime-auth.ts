...
import {
  cloneOAuthCredential,
  parseCredentialRequest,
} from "./oauth-credential-source.ts";
...
export class RuntimeAuthCoordinator {
  ...
  async ensureActive(
    ...
    if (!credential) {
      ...
      return this.failClosed(
        ...
        active,
        new Error(
          `${this.provider.displayName} account "${active}" is no longer available.`,
        ),
      );
    }
    ...
    if (credential.expires <= now + REFRESH_SKEW_MS) {
      ...
      try {
        current = await store.updateProviderAsync(
          ...
          async (latest) => {
            ...
            try {
              refreshSignal.throwIfAborted();
              const refreshed = await resolveProviderOAuth(
                this.provider,
                ctx,
              ).refresh(latestCredential, refreshSignal);
              refreshSignal.throwIfAborted();
      ...
      if (refreshError !== undefined) {
        const selection = await this.selectedCredentialMatches(
          store,
          active,
          credential,
        );
    ...
    try {
      ...
      const refreshedSelection = await this.selectedCredentialMatches(
        store,
        active,
        credential,
      );
  ...
  publishCredentialOffer(
    ...
    if (
      activeIdentity !== `${pending.accountName}:${pending.credential.access}`
    )
      return;
  ...
  offerCredential(data: unknown): void {
    ...
    if (
      ...
      request.provider !== this.provider.id
    )
      return;
  ...
  private beginRefreshOperation(): AbortSignal {
    this.refreshController.abort(
      new DOMException("Account operation superseded", "AbortError"),
    );
  ...
  private async refreshModelCatalog(
    ...
    if (this.catalogAuthIdentity !== authIdentity) {
      ...
      if (result.aborted) {
        throw new Error(
          `${this.provider.displayName} model catalog refresh was aborted.`,
        );
      }
  ...
  private async rebindRefreshedModel(
    ctx: ExtensionContext,
    operation: number,
  ): Promise<void> {
    if (
      !this.provider.refreshModelCatalogAfterAuth ||
      ctx.model?.provider !== this.provider.id
    )
      return;
  ...
  invalidate(ctx: ExtensionContext, resetCatalogIdentity = true): void {
    ...
    this.refreshController.abort(
      new DOMException("Account refresh invalidated", "AbortError"),
    );
    this.refreshController = new AbortController();
  }
...
class RuntimeProviderOverlay {
  ...
  constructor(provider: AccountProviderAdapter, fallbackApiKey: string) {
    this.provider = provider;
...
function readProviderModels(
  ...
  return models
    .filter(
      (model) => model.provider === providerId && typeof model.id === "string",
    )
    .map((model) => ({
      id: String(model.id),
      ...
      ...(model.thinkingLevelMap
        ? { thinkingLevelMap: model.thinkingLevelMap as never }
        : {}),
      input: Array.isArray(model.input)
        ? (model.input as ("text" | "image")[])
        : ["text"],
      cost: (model.cost ?? {
        input: 0,
        output: 0,
...
function validateModelAuth(
  ...
  if (auth.apiKey !== undefined) {
    throw new Error(
      `${providerName} OAuth returned unexpected API-key authentication.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `${providerName} OAuth returned no valid Bearer authorization header.`,
    );
  }
...
function findProviderModel(
  ...
  return typeof registry.find === "function"
    ? registry.find(providerId, modelId)
    : undefined;
}
