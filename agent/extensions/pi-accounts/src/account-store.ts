...
export class AccountStore {
  ...
  constructor(backend: AccountStorageBackend = createDefaultBackend()) {
  ...
  async update(
    mutator: (data: AccountsData) => AccountsData,
  ...
  async updateAsync(
    ...
    return this.serialized(async () =>
      this.backend.withLockAsync(async (current) => {
        ...
        return {
          result: normalizeAccountsData(next),
          next: stringifyAccountsData(next),
        };
      }),
    );
  }

  async readProviderAsync(
  ...
  async updateProvider(
    ...
    return this.updateProviderAsync(providerId, async (state) =>
      mutator(state),
    );
  }
  ...
  async writeRawForTest(raw: string): Promise<void> {
    await this.serialized(async () =>
      this.backend.withLockAsync(async () => ({
        result: undefined,
        next: raw,
      })),
    );
...
function normalizeAccountsData(value: unknown): AccountsData {
  if (!isRecord(value))
    throw new Error("Invalid accounts data: expected an object.");
  if (value.version !== 1)
    throw new Error("Invalid accounts data: version must be 1.");
...
function normalizeProviderState(value: unknown): ProviderAccountsData {
  if (!isRecord(value))
    throw new Error("Invalid accounts data: provider state must be an object.");
  ...
  for (const [name, credential] of Object.entries(value.accounts)) {
    ...
    if (!parsedName.ok)
      throw new Error(`Invalid accounts data: bad account name "${name}".`);
...
export function normalizeStoredCredential(
  ...
  if (typeof cloned.access !== "string" || !cloned.access) {
    throw new Error(
      `Invalid accounts data: ${accountName} credential is missing access token.`,
    );
  }
  if (typeof cloned.refresh !== "string") {
    throw new Error(
      `Invalid accounts data: ${accountName} credential is missing refresh token.`,
    );
  }
  if (typeof cloned.expires !== "number" || !Number.isFinite(cloned.expires)) {
    throw new Error(
      `Invalid accounts data: ${accountName} credential has invalid expiration.`,
    );
  }
  ...
  try {
    if (Array.isArray(value)) {
      return value.map((entry, index) =>
        cloneJsonValue(entry, seen, `${path}[${index}]`),
      );
    }
...
function migrateReleasedCodexData(raw: string | undefined): AccountsData {
  ...
  try {
    ...
    throw new Error(
      `Invalid legacy Codex accounts JSON in ${LEGACY_CODEX_ACCOUNTS_FILE}.`,
    );
  }
...
function cleanupStaleMigrationTemps(canonicalPath: string): void {
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (Date.now() - info.mtimeMs >= MIGRATION_TEMP_STALE_MS)
...
function emptyAccountsData(): AccountsData {
  return {
    version: 1,
    providers: Object.create(null) as Record<string, ProviderAccountsData>,
  };
}
...
function cloneProviderState(
  state: ProviderAccountsData | undefined,
...
function parseActiveAccount(value: unknown): string | undefined {
  ...
  if (typeof value !== "string")
    throw new Error("Invalid accounts data: active must be a string.");
