...
const syncSleepState = new Int32Array(
...
export function createLockfileFsAdapter(
  source: LockfileFsAdapter,
): LockfileFsAdapter {
  return {
    mkdir: source.mkdir,
...
export interface AccountStorageBackend {
  ...
  withLock<T>(
    mutator: (current: string | undefined) => StorageLockResult<T>,
  ): T;
  withLockAsync<T>(
...
export class FileAccountStorageBackend implements AccountStorageBackend {
  ...
  constructor(filePath: string, options: { syncLockTimeoutMs?: number } = {}) {
  ...
  async readAsync<T>(
    reader: (current: string | undefined) => Promise<T>,
  ): Promise<T> {
    if (!this.fileOrLockExistsForRead()) return reader(undefined);
    ...
    try {
      ...
      const result = await reader(
        readPrivateRegularFileIfExists(this.filePath),
      );
      throwIfCompromised();
      return result;
    } finally {
  ...
  private acquireLockSyncWithRetry(): () => void {
    const timeoutMs =
      this.options.syncLockTimeoutMs ?? DEFAULT_SYNC_LOCK_TIMEOUT_MS;
    ...
    while (true) {
      try {
        ...
        Atomics.wait(
          ...
          0,
          0,
          Math.min(SYNC_LOCK_RETRY_INTERVAL_MS, remainingMs),
        );
      }
    }
  }
